...
module.exports = function getExpeditiousCacheMiddleware (opts) {
  ...
  const cache = opts.expeditious = opts.expeditious || expeditious({
    namespace: opts.namespace,
    defaultTtl: opts.defaultTtl,
    objectMode: true, // this has to be true
  ...
  const cacheLocks = require('./cache-locks')(opts);
  ...
  const middleware = function (req, res, next) {
    const log = require('./log')(req);
    const cacheKey = genCacheKey(req);

    // This will be determined based on the url and if a request to this
    // url is already in progress by another user
    ...
    function onCacheReturned (err, httpData) {
      ...
      if (err) {
        ...
        log('cache hit. responding with cached data');
    ...
    function respondWithCache (httpData) {
      ...
      if (req.fresh) {
        ...
        log('returning cached data, etag did not match');
        ...
        res.socket.write(httpData.completeHttpBody);
        res.end();
      }
    }
    ...
    function onResponseFinished (err, httpData) {
      ...
      if (err) {
        log('error processing request, not storing response in cache', err);
    ...
    function proceedWithRequest () {
      ...
      res.socket.write = (function (res) {
        ...
        return function customWrite (body) {
          ...
          // matching request is already being written to the cache. If there
          // is then we won't attempt to cache again since it could have a large
          // performance penalty in high concurrency environments by creating
          // too many buffers
          ...
          if (isInitialWrite) {
            ...
            // TODO: what about clusters? future feature that might be required
  ...
  // expose cache instance for easier referencing
  middleware.expeditious = cache;
