...
const getMiddlewareInstance = module.exports = function (opts) {
  ...
  const expeditiousMiddleware = function (req, res, next) {
    ...
    function processRequest () {
      ...
      // the cache since we have everything we need to do so. Also need to
      // listen for the aborted event so we don't accidentally store
      // empty/partial caches
      req.on('aborted', () => {
        // this is required for node v6, in node v8 we can use the
        // "req" param passed to "onResponseFinished"
        aborted = true
      })
